...
例）枠が赤の点線で角が丸く、文字は白で影を付けて中央寄せされており、背景は青のグラデーション、としたい場合のCSS定義
```css
.Sample {
...
プロパティの仕様（あるプロパティが指定された際に画面にどう描画されるか）は標準化団体により[定義](https://www.w3.org/Style/CSS/specs.en.html)されています。  
...
指定方法の中では、クラス名で指定する方法が扱いやすくよく使われます。  
