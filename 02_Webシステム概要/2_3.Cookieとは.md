...
保持する際は、リクエストURLのドメインと紐付けて保存されます。  

2度目以降のレスポンスで同じCookie名が返ってきた場合は、保存している情報を上書きします。  


### リクエストヘッダー
保存済みのCookieがあり、そのCookieに紐付けられたドメインのサーバーへリクエストを送信する場合、保存済みのCookieの値がブラウザによってリクエストヘッダーに記載されます。[^1]  

[^1]:Cookieを送信するドメインは、後述するdomain属性で変更可能です。  
デフォルトの場合はCookie取得時のURLのドメインに対してのみCookieを送信します。  
