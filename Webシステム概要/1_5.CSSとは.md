...
例えば、ある要素の背景色を指定する場合は以下のように記載します。    
```html
...
例）枠が黄色の点線で角を丸くする、文字は白で影を付ける、背景は青のグラデーション
```css
...
プロパティの仕様（そのプロパティが指定された際に画面にどう描画されるか）は仕様策定団体により定義されています。  
...
この場合、**セレクタ**（後述）にてどのタグにスタイルを適用するかを指定します。  
...
②、③の方法で記載する場合は、***セレクタ***でどの要素にどの定義を適用するかを指定します。  


## セレクタ
要素外にCSSを定義する場合、どの要素にCSSを適用するかを表す**セレクタ**を指定する必要があります。   

セレクタとは、CSS定義を適応する対象の要素を指定するための書式です。（後述）  
...
例えば、h1要素の背景色をと高さを変更する場合は以下のように指定します。  
```css
h1 {
  background-color: blue;
  height: 20px;
...
```
この例だと全てのh1要素に2つのプロパィ定義が適用されます。  

このような定義をHTMLの`<style>`タグに直接記述するか、外部ファイルに記述し`<link>`タグで参照することで、セレクタの条件に合致する要素にCSSの定義が適用されます。  
...
## padding、margin
ブロックボックスという種別のHTML要素は、以下の部分で構成されています。  
...
- Border box  
 境界ボックスは、コンテンツとパディングをラップします。そのサイズとスタイルは、border および関連するプロパティを使用して制御できます。
- Margin box  
マージンは最も外側のレイヤーで、このボックスと他の要素の間の空白としてコンテンツ、パディング、および境界線をラップします。そのサイズは、margin および関連するプロパティを使用して制御できます。
