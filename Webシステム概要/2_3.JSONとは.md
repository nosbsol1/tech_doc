...
(以前はxmlがよく利用されていましたが、2022年現在は文法がシンプルなJsonが利用されることが多いです。)  
...
1. （C#）クライアントに返却する値を持ったオブジェクトをJson文字列に変換
...
const search = async () => {
    ...
    //サーバーからのレスポンスボディのJsonをJavascriptのオブジェクトに変換
...
※実際にはこの例のようにボディから取得するコードを記述することはありません。  
（フレームワーク側が変換してくれます。）  
処理がイメージしやすいようサンプルで書いてます。  
