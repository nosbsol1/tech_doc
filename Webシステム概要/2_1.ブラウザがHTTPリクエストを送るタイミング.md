...
### Webページ全体の再読み込み  
この記事では、画面が一度真っ白になり、ブラウザのタブに読み込みマークが表示される遷移のことを「Webページ全体の再読み込み」と呼ぶことにします。  
別のURLへ遷移した際や、画面をリフレッシュした場合の動作の事です。  

この場合ブラウザは再度WebサーバーからHTMLなどを取得しなおし画面を再描画します。  
...
### `<form>`タグの仕様
`<form>`タグの中にある`type="submit"`属性が指定された`<button>`をクリックすると、formタグのaction属性に指定されたURLにHTTPリクエストを送信し遷移します。  
...
リクエスト先のURLに遷移しWebページ全体が再読み込みされます。 
（その為リクエスト先のURLはHTMLを返す必要があります。） 

例えば、登録画面からフォームをサブミットして、登録結果画面を表示する、などの使い方があります。  
<img src="画像/ブラウザがHTTPリクエストを送るタイミング_POSTの使用例.gif" style="width:240px;">  


フォームデータの送信時、**フォームコントロールに入力されたデータがリクエストに記載されます。**  
...
- GET  
  ...
  ただ、パラメータがURLとして見えてしまうため、機密情報や個人情報がパラメータに含まれるとセキュリティ的に問題があります。  
  （そのようなURLをブックマークすると、ブックマークからそれらのパラメータが漏れる可能性があります。  
...
- 画面上のURLが変化しない  
ブラウザ上のURLも変更されませんし、戻る・進むの履歴も変化しません。 

- サーバーからのレスポンスを待たない   
ページ全体を再読み込みする際は、サーバーからのレスポンスが来て画面が再描画されるまで画面上で操作を行えません。  
Javascriptからのリクエストの場合、Javascriptがレスポンスをまっている間も画面上で操作が可能です。  
その為、ブラウザを操作しているユーザーに気付かれずにリクエストを発行することが出来ます。  
この特性から、Javascriptから行うHTTP通信は**非同期通信**とも呼ばれます。  

- レスポンスに対しての処理をコードで記述する必要がある  
レスポンスに対してどのような処理を行うかもJavascriptのコードで記述する必要があります。  
...
（そのようなHTMLを操作する為のAPIを**DOM API**といいます。）  
...
<img src="画像/ブラウザがHTTPリクエストを送るタイミング_Ajaxの例.gif" style="width:360px;">    
...
Ajaxでのページ変更はページ全体の再読み込みが発生しない為、ユーザーからすると反応が早く感じます。（サクサク動いているように感じます）    

ページ全体を再読み込みするとページを1から描画しなおすため、画面表示に1～2秒掛かることがあり、ユーザーのストレスとなる可能性があります。  
このように要所でAjaxを用いた画面変更を行うことで、ユーザーの使い勝手が向上します。  
...
ボタンをクリックすると`<label>`タグの箇所にサーバーから返却された文字列が表示されるようなイメージです。  
